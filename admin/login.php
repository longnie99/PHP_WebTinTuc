...
if($_SERVER['REQUEST_METHOD'] == "POST") {
	...
	if (empty($error)) {
		$is_check = mysqli_fetch_assoc($db->query("SELECT * FROM adminn WHERE email = '$email' AND mat_khau = '$password'"));
	
		if (isset($is_check) != NULL) {

			$_SESSION['adminn_hoten'] = $is_check['ho_ten'];
			$_SESSION['adminn_id'] = $is_check['id'];
			$_SESSION['adminn_email'] = $is_check['email'];
